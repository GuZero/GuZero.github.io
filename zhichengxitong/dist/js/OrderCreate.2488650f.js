...
function dateFormat(y, m, d, h, fm) {
    ...
    d = ('0' + d).slice(-2);
    return fm.replace('yyyy', y).replace('YYYY', y).replace('mm', m).replace('MM', m).replace('DD', d).replace('dd', d).replace('HH', h).replace('hh', h);
}
// 比较两个日期大小，返回 -1 0 1
    ...
    methods: {
        ...
        choiceDay: function choiceDay(d) {
            ...
            if (d && this.dateIsValid({
                ...
                this.day = d;
                var hh = this.yourTimeValue.HH;
                this.$emit('input', dateFormat(year, month, d, hh, format));
                console.log(hh);
                var date = this.value.substring(0, 10),
                    time = '';
                time = date + ' ' + this.yourTimeValue.HH + ':' + this.yourTimeValue.mm;
                this.value = time;
                this.immEndChoice();
            } else {
                var _date = this.value.substring(0, 10),
                    _time = '';
                _time = _date + ' ' + this.yourTimeValue.HH + ':' + this.yourTimeValue.mm;
                this.value = _time;
            }
        },
    ...
    methods: {
        submitFun: function submitFun() {
            ...
            var project_id = this.project_id,
                ...
                appearance = this.val,
                timeout = this.date + ' ' + localStorage.HH + ':' + localStorage.MM,
                state = this.state,
                content = this.desc;
            this.date = timeout;
