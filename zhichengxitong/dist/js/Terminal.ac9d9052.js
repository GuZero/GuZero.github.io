...
/* harmony default export */ exports["default"] = _mixins$data$created$ = {
...
}, _defineProperty(_mixins$data$created$, 'activated', function activated() {
    //开启<keep-alive>，会触发activated事件
    this.resetScrollTop();
    window.addEventListener('scroll', this.handleScroll);
}), _defineProperty(_mixins$data$created$, 'beforeRouteLeave', function beforeRouteLeave(to, from, next) {
    //离开当前路由触发该事件
    //保存滚动条位置，卸载滚动加载事件
    ...
    next();
}), _defineProperty(_mixins$data$created$, 'methods', {
    ...
    loadCitys: function loadCitys(data, e) {
        ...
        that.citys = [];
        that.showLoading();
        that.getAjaxRequest("terminal_cache", ajaxUrls.citys + data.ar_id, that.version, 2 * 60 * 1000, 6 * 60 * 60 * 1000, function (response) {
            that.hideLoading();
    ...
    loadAreaData: function loadAreaData() {
        ...
        if (that.areas && !that.areas.length) {
            that.showLoading();
            that.getAjaxRequest("terminal_cache", ajaxUrls.areas, that.version, 2 * 60 * 1000, 6 * 60 * 60 * 1000, function (response) {
                that.hideLoading();
                if (response.status == 0) {
    ...
    loadTerminalData: function loadTerminalData(isFirst) {
        ...
        that.getAjaxRequest("terminal_cache", ajaxUrls.terminals + that.ar_id + '/citys/' + that.city_id + '?page=' + page, that.version, 2 * 60 * 1000, 6 * 60 * 60 * 1000, function (response) {
            that.hideLoading();
    ...
    searchTerminal: function searchTerminal(isFirst) {
        ...
        that.tn_delay = setTimeout(function () {
            ...
            that.getAjaxRequest("terminal_cache", ajaxUrls.searchTerminal + _key.trim() + '&page=' + page, that.version, 2 * 60 * 1000, 6 * 60 * 60 * 1000, function (response) {
                that.hideLoading();
                that.tn_scroll_load_loading = false;
                if (response.status == 0 && response.data && response.data.length) {
