    ...
    var lCalendar = (function() {
        ...
        MobileCalendar.prototype = {
            init: function(params) {
                ...
                if (params.countDay) this.countDay = params.countDay;
                ...
                _thisCallBack = this.callBack;
                _this = this;
                this.bindEvent(this.type, _this);

            },
            bindEvent: function(type, _this) {
                var _self = this;
                //呼出日期插件
                function popupDate(e) {
                    ...
                    _self.gearDate.innerHTML = '<div class="date_ctrl slideInUp" id="calender">' +
                ...
                function setDateGearTooth() {
                    ...
                    var date_YY = _self.gearDate.querySelector(".date_yy");
                    var index = parseInt(_self.gearDate.querySelector(".date_yy").getAttribute("val"));
                    //如果在同一年内
                    if (new Date().getFullYear() == (index + '').slice(1)) {
                        _this.curr.yy = (index + '').slice(1);
                    } else {
                        _this.curr.yy = date_YY.children[index].innerText;
                    }
                    endCallBack.call(_this, _this, _this.curr);
                ...
                //滑动完回掉
                function endCallBack(_this, curr) {
                    _this.callBack(_this, curr);
                }
                //结束时间选择完成后的回调
                function endTmFinishCallBack() {
                    if (_this.countDay) {
                        _this.countDay();
                    }
                }
