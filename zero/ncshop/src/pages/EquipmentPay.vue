            ...
            <div class="pay-box">
                ...
                <div style="padding:16px;">
                    <div class="item flex-g" v-for="item in items" :key="item.id">
                        <div style="width:70%">{{item.trading.title}}</div>
                        <div style="width:10%;text-align:center;" class="c-gary">×{{item.num}}</div>
                        <div style="width:20%;text-align:right;">{{item.trading.price/100*item.num}}元</div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        getInfo() {
            let that = this;
            $('#sysLoading').show();
            axios.get('/ncshop/equipment/cart/' + that.$route.query.cart_id)
                .then(function(res) {
                    $('#sysLoading').hide();
                    if (res.data.status == 0) {
                        ...
                        for (let i = 0; i < data.length; i++) {
                            that.sum += data[i].num * data[i].trading.price / 100;
                            let obj=Object.create(null);
                            obj.trading_id=data[i].trading_id;
                            obj.price=data[i].trading.price;
                            obj.num=data[i].num;
                ...
                wx.chooseWXPay({
                    ...
                    "success": function(res) {
                        ...
                        that.ajaxPay(data, function(data) {
                            ...
                            $('#sysLoading').hide();
                            that.url('/ncshop/equipment/pay/success', {order_id: order_id}, true);
                        });
                    },
                    "cancel": function(res) {
                        var data = {
        ...
        submitPay(order_id){
            ...
            if(pay_type == 4){
                ...
                if(window.ncshop_config && (window.ncshop_config.equipment_alipay_account || window.ncshop_config.alipay_account)){
                    data.account = window.ncshop_config.equipment_alipay_account || window.ncshop_config.alipay_account
                }
            }
