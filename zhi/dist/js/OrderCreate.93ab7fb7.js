    ...
    methods: {
        ...
        setData: function setData(strMap) {
            for (var i = 0; i > strMap.lenght; i++) {
                ...
                try {
                    for (var _iterator = Object.keys(strMap)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                        var id = _step.value;
        ...
        getInfo: function getInfo() {
            ...
            getAjaxRequest("order_cache", ajaxUrls.option, that.version, 20 * 1000, 0.5 * 60 * 60 * 1000, function (response) {
                ...
                if (response.status == 0) {
                    //                        that.ordertypes=Array.from(response.project);
                    //                        console.log(that.ordertypes);
                    var arr = response.type['1']['0'];

                    that.scenes = [].concat(_toConsumableArray(arr));
